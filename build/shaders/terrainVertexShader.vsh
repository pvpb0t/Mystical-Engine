...
//MAKE THIS UNIFORM VAR to change it with config
const float density = 0.003;
const float gradient = 0.57;
...
void main(void){
	...
	pass_textureCoordinates = textureCoordinates;      //*texture quality
	surfaceNormal = (transformationMatrix * vec4(normal, 0.0)).xyz; //(swizzle it) convert from vec4 back to vec3
	toLightVector = lightPosition - worldPosition.xyz;
	toCameraVector = (inverse(viewMatrix)*vec4(0.0,0.0,0.0,1.0)).xyz - worldPosition.xyz;
	float distance = length(positionRelativeToCam.xyz); //How far away texture is from camera
